...
export class NotificationsService {
  ...
  async list(userId: number): Promise<{ unreadCount: number; notifications: StaffNotificationRecord[] }> {
    const [itemsResult, unreadResult] = await Promise.all([
      this.pool.query<NotificationRow>(
        `${SELECT_NOTIFICATIONS} WHERE n.user_id = $1
         ORDER BY n.created_at DESC, n.id DESC LIMIT 50`,
        [userId],
      ),
      this.pool.query<{ count: number }>(
        "SELECT COUNT(*)::int AS count FROM staff_notifications WHERE user_id = $1 AND read_at IS NULL",
        [userId],
      ),
    ]);
    return {
  ...
  async markRead(id: number, userId: number): Promise<StaffNotificationRecord> {
    const client = await this.pool.connect();
    try {
      await client.query("BEGIN");
      const beforeResult = await client.query<NotificationRow>(
        `${SELECT_NOTIFICATIONS} WHERE n.id = $1 AND n.user_id = $2 FOR UPDATE OF n`,
        [id, userId],
      );
