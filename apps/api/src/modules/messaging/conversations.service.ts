...
export class ConversationsService {
  ...
  async findOrCreateConversation(jid: string, phoneNumber: string): Promise<number> {
    const existing = await this.pool.query<{ id: number }>(
      "SELECT id FROM conversations WHERE whatsapp_jid = $1",
      [jid],
    );
    ...
    const inserted = await this.pool.query<{ id: number }>(
      "INSERT INTO conversations (whatsapp_jid, phone_number) VALUES ($1, $2) RETURNING id",
      [jid, phoneNumber],
    );
  ...
  async listConversations(): Promise<ConversationSummary[]> {
    const result = await this.pool.query<{
      ...
      `SELECT c.id, c.whatsapp_jid, c.phone_number, c.last_message_at,
  ...
  async getConversation(id: number): Promise<ConversationRecord | null> {
    const result = await this.pool.query<{ id: number; whatsapp_jid: string; phone_number: string }>(
      "SELECT id, whatsapp_jid, phone_number FROM conversations WHERE id = $1",
      [id],
    );
    const row = result.rows[0];
    if (!row) return null;
    return { id: row.id, whatsappJid: row.whatsapp_jid, phoneNumber: row.phone_number };
  }
