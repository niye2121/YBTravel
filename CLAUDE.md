...
A live WhatsApp inbox now exists (`apps/api/src/modules/messaging/`, `apps/web/src/routes/inbox.tsx`) — the first real domain tables (`whatsapp_connections`, `conversations`, `messages`, still raw SQL, no ORM), the first live external integration (Baileys, on a separate/test number per the resolved decision in `docs/05-open-decisions.md` #9), and the first use of the WebSocket gateway and TanStack Query. All messaging REST endpoints and Socket.IO connections require the current application JWT; the WebSocket handshake re-reads the user from the database before admitting the connection.
